...
**Stack:** Python (pandas/duckdb) → DuckDB → dbt → Streamlit (próxima fase).
...
Clasificación (`artist_scouting.ar_recommendation`):
...
│       │   └── stg_artists.sql
│       └── marts/
│           └── artist_scouting.sql # Scouting Score + clasificación A&R
├── requirements.txt
├── setup.sh
└── README.md
```
